...
function Switch({ x, set }) {
  const { nodes, materials } = useGLTF('/assets/3d/switch.glb') as GLTFResult
  const texture = useTexture('/assets/images/cross.jpg')
  // Hover state
...
export default function Scene({ x, set }) {
  // Create a color interpolation
  const color = x.to([0, 1], ['#FDDD82', '#fff'])
  const position = new THREE.Vector3(-10, 10, 10)
  return (
    <Canvas orthographic shadows dpr={[1, 2]} camera={{ zoom: 60, position, fov: 35 }}>
